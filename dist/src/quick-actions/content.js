...
(function() {
  ...
  /**
   * 設定を読み込んで MENU_ITEMS をフィルタリング
   */
  async function loadQuickActionsConfig() {
    try {
      const res = await chrome.storage.local.get({
        quickActionsConfig: DEFAULT_QUICK_ACTIONS_CONFIG,
      });
      const config = res.quickActionsConfig || DEFAULT_QUICK_ACTIONS_CONFIG;

      // 設定に基づいて MENU_ITEMS をフィルタリング
      MENU_ITEMS = ALL_MENU_ITEMS.filter(item => config[item.id] !== false);

      console.log('Keep Quick Actions: 設定を読み込みました。有効なアクション:', MENU_ITEMS.map(m => m.id));
    } catch (err) {
      console.error('Keep Quick Actions: 設定の読み込みに失敗しました', err);
      MENU_ITEMS = [...ALL_MENU_ITEMS]; // デフォルトを使用
    }
  }
  ...
  function watchStorageChanges() {
    chrome.storage.onChanged.addListener((changes, areaName) => {
      ...
      if ('quickActionsConfig' in changes) {
        // 設定が変更されたら再度読み込む
        loadQuickActionsConfig().then(() => {
          // ページ再読み込みが必要（即座に反映するため）
          console.log('Keep Quick Actions: 設定が変更されました。ページを再読み込みしています。');
          // 実際には、content.jsの再読み込みが必要なので、ここではログのみ
        });
      }
    });
  }
